...
current_duration = 0 # 当前检测持续时间（ms）
...
while True:
    ...
    print(f"是否识别到粪便：{found_class_2}，{fail_send_count}，{fail_send_limit}")
    ...
    shidu =66
    # 发送给 LCD 显示
    #rlcd.write(img,  x_scale=scale_x,y_scale=scale_y,  )
      # 拍照并显示图像。
    img.draw_string(0, 0, f"number of shit: {shit_detected_count}", color=(0, 255, 0), thickness=2)  # 如果支持 f-string
    img.draw_string(0, 20, f"number of pig: {pig_detected_count}", color=(0, 255, 0), thickness=2)  # 如果支持 f-string
