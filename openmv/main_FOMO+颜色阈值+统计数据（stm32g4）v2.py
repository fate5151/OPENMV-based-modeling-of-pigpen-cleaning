...
while True:
    ...
    for i, detection_list in enumerate(net.predict([img], callback=fomo_post_process)):
        ...
        if i == 2:
            found_class_2 = True  # 标记已找到第二类目标
            daily_target_count += 1
            daily_label_1_detects += len(detection_list)

            for x, y, w, h, score in detection_list:
                color_x = max(0, int(x - w))
                color_y = max(0, int(y - h))
                color_w = 3 * w
                color_h = 3 * h
                color_roi = (color_x, color_y, color_w, color_h)
                img.draw_rectangle(color_roi, color=(0, 0, 255))  # 蓝色辅助框
                ...
                if blobs:
                    ...
                    if target_blob:
                        ...
                        img.draw_rectangle(target_blob.rect(), color=(255, 0, 0))  # 红框
                        now = pyb.millis()
                        ...
                        daily_red_detect_count += 1
    if not found_class_2:
        data1 = bytearray([0xf1, 0, 0xe1])
        uart.write(data1)
        daily_uart_send_count += 1
        daily_fail_count += 1
        print(f"未检测到红色色块，发送0: {data1}")
